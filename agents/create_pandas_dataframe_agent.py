...
from agents.format_instructions_prompt import FORMAT_INSTRUCTIONS
...
from agents.pandas_agent_prompts import (
    FUNCTIONS_WITH_DF,
...
def _get_single_prompt(
    ...
    *,
    ...
    if suffix is not None:
        ...
        # For large dataset, just print out some of the columns
        if len(df.columns) > 20:
            suffix_to_use = SUFFIX_WITH_DF_LARGE
        else:
    ...
    if "df_cols" in partial_prompt.input_variables:
        df_cols = str(df.columns)
        partial_prompt = partial_prompt.partial(df_cols=df_cols)
...
def _get_functions_single_prompt(
    ...
    *,
    ...
    if include_df_in_prompt:
        df_head = str(df.head(number_of_head_rows).to_markdown())
        df_cols = str(df.columns.to_markdown())
        if len(df.columns) > 20:
            print("FLAG AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
            suffix = (suffix or FUNCTIONS_WITH_DF_LARGE).format(df_cols=df_cols)
        else:
            print("FLAG BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB")
...
def _get_functions_multi_prompt(
    ...
    *,
    ...
    if include_df_in_prompt:
        dfs_head = "\n\n".join([d.head(number_of_head_rows).to_markdown() if len(d.columns) <=20 else d.columns.to_markdown() for d in dfs])

        print("FLAG CCCCCCCCCCCCCCCCCCCCCC")
        print(dfs[0].columns.to_markdown())
        print(dfs_head)
